...
patches-own [
  ...
  toxic                ; patches will become toxic if a certain amount of drug comes into contact with that patch - will lead to cell death
]
...
to Run-Simulation
  ...
  ;ifelse ShowToxicity                               ;set patches to report toxicity by color
  ;[ask patches with [wound = 1 and toxic > 0][set pcolor scale-color green toxic 0 4]]
  ...
  ;ask patches with [wound = 1 and toxic > 5][set pcolor red - 3]
  ask patches with [wound = 1 and debris = 0][set pcolor red]
  ask patches with [wound = 1 and toxic > 5][set pcolor red - 3] ;reset patch color to red or black based on previous wound state
  ...
  ask drugs with [age > 256][if 100 * (age / 512) > random 100 [die]]
  ask patches [if debris < 0 [set debris 0]]
  ask patches [if toxic < 0 [set toxic 0]]
...
to Chemo
  ask patch 0 0 [
    ...
    ask n-of Chemotherapy-Dose patches in-radius 15[
      sprout 1        ; create the number of drug units as designated by the slider on the interface tab
      [
        set breed drugs              ; denote the 'breed' of the turtle as an "drug" agent
        set shape "x"            ; denote the shape of the turtle as a drug shape
        set color green
        if chemo-half-life = "Slow" [set age 0]
        if chemo-half-life = "Normal" [set age 10]
        if chemo-half-life = "Fast" [set age 20]
      ]
    ]
  ]
end
...
to migrate
  ...
  [
    ...
    ask drugs
    [
      if any? neighbors with [not any? drugs-here]            ; migration only occurs if there is at least one empty neighboring patch
      [
        move-to one-of neighbors with [not any? drugs-here] ; migrate to one of the 8 neighboring patches without drug in it already
      ]
    ]
  ]
  ...
  ask neutrophils ;neutrophils move towards bacteria in their homing range, or to a random neighboring patch
  [
...
to Leukocyte-Actions
  ...
  ask neutrophils
  [
    if [toxic] of patch-here > 5[
      ask neutrophils-on patch-here[die ask patch-here [set debris debris + 1]]
    ]
  ]
  ...
  ask drugs [if any? neutrophils-on patch-here
    [
      ask neutrophils-on patch-here[die ask patch-here [set debris debris + 1]]
    ]
  ]
  ask drugs [if any? drugs-on patch-here
    [
      ask patch-here [set toxic toxic + 1]
    ]
  ]
...
415
645
Percent of wound containing debris and toxicity
NIL
NIL
0.0
200.0
...
"high toxicity" 1.0 0 -2139308 true "" "plot count patches with [pcolor = 12] / 709 * 100 "
...
648
644
Percent of wound with high toxicity
count patches with [pcolor = 12] / 709 * 100
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Project Data" repetitions="5" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup
infect</setup>
    ...
    <metric>count patches with [pcolor = 12] / 709 * 100</metric>
    ...
    <enumeratedValueSet variable="Chemotherapy-Dose">
      <value value="0"/>
      <value value="10"/>
      <value value="20"/>
      <value value="30"/>
